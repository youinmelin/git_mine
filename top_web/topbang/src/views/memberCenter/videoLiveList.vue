        ...
        <div v-for="(item,index) in list" :key="index" class="cell">
          ...
          <div class="bot">
            ...
            <div class="col-3">
              ...
              <div class="grey curpp" @click="$router.push({path:'/videoLiveMain',query:{id:item.id}})">直播详情</div>
            </div>
            <div class="col-4">
              ...
              <el-button
                ...
                type="text"
                @click="$router.push({path:'/memberCenter/videoLive',query:{id:item.id}})">重新编辑
              </el-button>
              <el-button v-if="item.status===2" type="text">下架视频</el-button>
              <el-button type="text" @click="deleteVideo(item.id)">删除直播</el-button>
            </div>
          </div>
        </div>
      </div>
...
<script>
import { deleteVideoLive, getVideoLiveList } from '@/api/product'
  ...
  methods: {
    ...
    deleteVideo(prodId) {
      this.$confirm('此操作将永久删除该直播, 是否继续?', '提示', {
        ...
        deleteVideoLive(prodId).then(response => {
    ...
    fetchData() {
      // 获取直播列表
      getVideoLiveList(this.listQuery).then(response => {
        if (response.data.code === 0) {
