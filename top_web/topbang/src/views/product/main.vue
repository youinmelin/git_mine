  ...
  <div class="product-main">
    ...
    <div class="main-content">
      <div style="width: 1200px; margin: 0 auto;">
        ...
        <div class="center">
          <div style="width: 100%;height: 540px;" v-for="(item,index) in playerOptions" :key="index">
            <video-player
    ...
    <div class="product-detail">
      <div class="pd-left">
        ...
        <div class="pd-desc">
          ...
          <!--              <span>好评率</span>-->
          <!--              <span class="gc-name-rate">99.8%</span>-->
          <!--            </div>-->
          <!--            <div class="gd-tags">-->
          <!--              <span-->
          ...
          <!--                @click="activeTag = index">{{ tag }}</span>-->
          <!--            </div>-->
          <!--          </div>-->
          <!--          <div class="comment-type">-->
          <!--            <span-->
          <!--              v-for="(type, index) in otherList"-->
          <!--              :key="index"-->
          <!--              class="type"-->
          <!--              :class="{'active': activeType === index}"-->
          <!--              @click="activeType = index">{{ type }}</span>-->
          <!--            <span style="float: right">-->
          <!--              <el-checkbox v-model="checked">只看当前商品评价</el-checkbox>-->
          <!--            </span>-->
          <!--          </div>-->
          <!-- 评论模块 -->
          <div
            class="pd-content"
            v-for="(item,index) in evelList"
            :key="index">
            <div class="user">
              <img class="avatar" src="../../assets/pics/sample2.jpg"/>
            ...
            <div class="user-comment">
              ...
              <div class="c-foot">
                <span>{{ item.productName }}         {{ item.gmtCreate }}</span>
                <!--                <span class="c-operate">-->
                <!--                  <span class="co jb">举报</span>-->
                <!--                  <span class="co">-->
                <!--                    <img src="../../assets/icons/dianzan.png"/>0-->
                <!--                  </span>-->
                <!--                  <span class="co">-->
                <!--                    <img src="../../assets/icons/comment.png"/>0-->
                <!--                  </span>-->
                <!--                </span>-->
              </div>
            </div>
            ...
            <div class="reply" v-if="item.replyContent !=null">
              <div>店家回复：</div>
              <div>{{ item.replyContent }}
              </div>
              <div class="reply-time">{{ item.gmtReply }}</div>
            </div>
          </div>
          <!-- 分页 -->
      ...
      <div class="pd-right">
        ...
        >
          ...
          <span class="bottom">
            <span class="time">{{ item.strDuration }}</span>
          </span>
        </div>
      </div>
    </div>
...
import { getWebPageProductEvaluate, getProductInfo, productGiveTrial, productCollection } from '@/api/product'
    ...
    getProductInfo() {
      getProductInfo(this.$route.query.id).then(response => {
        if (response.data.code === 0) {
          ...
          // 指定视频播放器的SRC
          // this.playerOptions.sources[0].src = this.videoList[0].videoUrl
          // this.playerOptions.poster = this.productInfo.videoImgUrl
          for (const i of this.videoList) {
            const arrStr = {
              ...
              sources: [{
                type: 'video/mp4', // 类型
                src: i.videoUrl // url地址
              }],
              poster: this.productInfo.videoImgUrl // 封面地址 http://yujia.img.cdn.chabug.com/test/image/video/20200913172458193000000259282398.png
            }
            this.playerOptions.push(arrStr)
          }
          // 给视频播放地址
          // this.playerOptions.sources[0].src = 'http://yujia.img.cdn.chabug.com/test/image/video/20200914175701982000000366588590.mp4'
          // 相关推荐
