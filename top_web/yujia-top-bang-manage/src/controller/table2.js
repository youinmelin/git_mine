...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    var $ = layui.$
        ...
        //字符常量
        , MOD_NAME = 'table2', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide',
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                , success: function (res) {
                    // 判断token
                    if (res[response.statusName] == 100006) {
                        admin.exit();
                    }
                    ...
                    if (res[response.statusName] != response.statusCode) {
                        ...
                        /* 修正分页总记录条数的参数读取 下面这一行由res[response.countName]改为res.data[response.countName]
                         * 虽然可以解决当前的问题，但并不是最优方案，空了再优化
                         * @author caorui
                         * @date 2019年01月28日09:57:42
                         */
                        that.renderData(res, curr, res.data[response.countName]), sort();
                        options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
                        console.log("当前页面加载耗时：" + options.time);
                    }
                    that.setColsWidth();
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        var that = this
            , options = that.config
            // 由于后端返回回来的地方，这里多包了一层。所以另起table2.js，然后修改了下面这一行代码 @author caorui 2019年01月25日16:11:27
            , data = res[options.response.dataName].list || []
            , trs = []
            , trs_fixed = []
            , trs_fixed_r = []
                        ...
                        openPanel({
                            list: function () {
                                return [
                                    ...
                                    // 2019年05月28日14:27:38 尝试优化导出功能未果，当前文件1864行也有一段新增内容
                                    , '<li data-type="txt">导出到 TxT 文件</li>'
                                ].join('')
                            }()
